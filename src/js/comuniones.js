...
async function cargarComuniones() {
  ...
  const { count } = await db.from('primeras_comuniones').select('*', { count: 'exact', head: true });
  totalRegistros = count || 0;
  const { data, error } = await db.from('primeras_comuniones').select('*').order('created_at', { ascending: false }).range(desde, desde + POR_PAGINA - 1);
...
async function buscar() {
  ...
  const { data } = await db.from('primeras_comuniones').select('*').or(`nombres.ilike.%${q}%,apellidos.ilike.%${q}%`).order('created_at', { ascending: false }).limit(50);
  todasLasComuniones = data || [];
  renderTabla(todasLasComuniones);
...
function limpiar() {
  ['f-nombres','f-apellidos','f-fechnac','f-lugarnac','f-sexo','f-fechcom',
   'f-padre','f-madre','f-ministro','f-anio-inicio','f-anio-fin','f-folio','f-notas']
